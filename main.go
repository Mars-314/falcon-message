...
func main() {
	...
	api.POST("/message", func(c echo.Context) error {
		...
		if args[1] == "PROBLEM" {
			tpl = template.Must(template.New(path.Base(cfg.DingTalk.TemplateFile_problem)).Funcs(funcMap).ParseFiles(cfg.DingTalk.TemplateFile_problem))
		} else if args[1] == "OK" {
			tpl = template.Must(template.New(path.Base(cfg.DingTalk.TemplateFile_ok)).Funcs(funcMap).ParseFiles(cfg.DingTalk.TemplateFile_ok))
		}
